...
import { SUPPORT_WHATSAPP, SUPPORT_WHATSAPP_LINK, MESSAGES, COLORS } from '../types';
...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || MESSAGES.LOGIN_ERROR);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <div className="text-center">
          ...
          <p className="mt-2 text-sm" style={{ color: COLORS.textMuted }}>
            Accede a tu cuenta de Academia Inteligente
          </p>
        </div>
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          {error && (
            <div className="rounded-lg p-4 text-sm" style={{ backgroundColor: '#fef2f2', color: COLORS.error, border: `1px solid ${COLORS.error}` }}>
              {error}
            </div>
          )}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              />
              <label htmlFor="remember-me" className="ml-2 block text-sm" style={{ color: COLORS.textMuted }}>
                Recordarme
              </label>
            </div>
